...
async def create_indexes():
    # Index for file queries by folder
    await db.files.create_index([("folder_id", 1)])
    # Index for folder hierarchy
    await db.folders.create_index([("parent_id", 1)])
    # Index for share tokens
    await db.shares.create_index([("token", 1)])
...
async def get_files(
    ...
    skip = (page - 1) * limit
    files = await db.files.find({'folder_id': folder_id}, {'_id': 0}).skip(skip).limit(limit).to_list(limit)
    total_files = await db.files.count_documents({'folder_id': folder_id})
